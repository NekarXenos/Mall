...
export function init(scene, camera, renderer, clock, controls) {
    ...
    // Attach camera directly to the head group
    headGroup.add(camera);
    camera.position.set(0, CAMERA_HEIGHT, CAMERA_DISTANCE); // Position relative to headGroup
    camera.lookAt(new THREE.Vector3(0, 0, 0)); // Look at the head's local origin
    ...
    return { gorillaGroup, headGroup, leftArmGroup, rightArmGroup, leftLegGroup, rightLegGroup, leftForearmGroup, rightForearmGroup, camera };
}


export function animate(scene, camera, renderer, clock, controls) {
