...
export class Mobster {
     ...
     * @param {Array<THREE.Object3D>} worldObjectsRef A reference to the global worldObjects array for collision management.
     */
    constructor(scene, initialPosition = new THREE.Vector3(0, 0, 0), floorIndex = 0, worldObjectsRef = []) {
        this.scene = scene;
        this.worldObjectsRef = worldObjectsRef; // Store the reference
        this.collisionMeshes = []; // To store meshes added to worldObjectsRef
        this.clock = new THREE.Clock();
        this.lastShotTime = 0;
        this.floorIndex = floorIndex;
        this.isFalling = false; // New state for falling animation
        this.fallRotationAxis = new THREE.Vector3();
        this.fallTargetQuaternion = new THREE.Quaternion();
        this.fallStartTime = 0;
        this.fallDuration = 0.8; // Duration of the fall animation
        this.initialY = initialPosition.y; // Store initial Y for landing
    ...
    _createCharacter() {
        ...
        this.characterGroup.traverse(object => {
            if (object.isMesh) {
                ...
                this.collisionMeshes.push(object); // Add to our list for removal
            }
        });
    ...
    update(deltaTime) {
        ...
        if (this.isFalling) {
            const progress = (elapsedTime - this.fallStartTime) / this.fallDuration;

            if (progress < 1) {
                // Interpolate rotation
                this.characterGroup.quaternion.slerp(this.characterGroup.quaternion, this.fallTargetQuaternion, progress);

                // Interpolate Y position to land on the floor
                // Assuming the mobster's feet are at its initialY when standing upright
                // And it falls to a new Y position (initialY - some_offset)
                const targetY = this.initialY - (this.characterGroup.scale.y * 0.8); // Adjust 0.8 based on mobster height
                this.characterGroup.position.y = THREE.MathUtils.lerp(this.initialY, targetY, progress);
            } else {
                this.isFalling = false;
                this.characterGroup.quaternion.copy(this.fallTargetQuaternion); // Ensure final rotation
                this.characterGroup.position.y = this.initialY - (this.characterGroup.scale.y * 0.8); // Ensure final Y position

                // After falling, start fading and remove
    ...
    takeDamage(amount, playerDirection) {
        this.health -= amount;
        if (this.health <= 0) {
            this.fallAndDisappear(playerDirection);
        }
    }

    fallAndDisappear(playerDirection) {
        this.isFalling = true;
        this.characterState = 'falling'; // New state

        // Calculate fall direction away from the player
        const fallDirection = this.characterGroup.position.clone().sub(playerDirection).normalize();
        fallDirection.y = 0; // Ensure falling horizontally first
        fallDirection.normalize();

        // Determine rotation axis (perpendicular to fall direction and up vector)
        this.fallRotationAxis.crossVectors(new THREE.Vector3(0, 1, 0), fallDirection).normalize();

        // Target rotation (90 degrees around the fallRotationAxis)
        this.fallTargetQuaternion.setFromAxisAngle(this.fallRotationAxis, Math.PI / 2); // 90 degrees

        this.fallStartTime = this.clock.getElapsedTime();

        // Remove collision meshes from worldObjectsRef immediately
        this.collisionMeshes.forEach(mesh => {
            const index = this.worldObjectsRef.indexOf(mesh);
            if (index > -1) {
                this.worldObjectsRef.splice(index, 1);
            }
        });

        // Animation loop for falling
        const animateFall = () => {
            if (!this.isFalling) return; // Stop if no longer falling
    ...
    fadeOutAndRemove() {
        ...
        this.characterGroup.traverse(object => {
            if (object.isMesh) {
                ...
                object.material.opacity = 1; // Start fully opaque
            }
        });

        const animateFade = () => {
            const elapsedTime = this.clock.getElapsedTime();
            const progress = (elapsedTime - startTime) / fadeDuration;
            ...
            if (progress < 1) {
                ...
                // Fully faded, remove from scene and dispose
