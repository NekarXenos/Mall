    ...
    for (let i = projectiles.length - 1; i >= 0; i--) {
        ...
        if (projectile.userData.firedByPlayer) {
            ...
            for (let j = enemies.length - 1; j >= 0; j--) {
                ...
                if (enemyBox.containsPoint(projectile.position)) {
                    enemy.takeDamage(100); // Each hit does 100 damage
                    if (enemy.health <= 0) {
                        // Mobster defeated
        ...
        if (!projectile.userData.firedByPlayer) {
            ...
            if (playerBox.intersectsBox(projectileBox)) {
                ...
                if (playerLives <= 0) {
                    isGameOver = true;
                    // Optional: Add game over logic here, like showing a message
                }
                continue; // Move to next projectile
            }
        }
...
function updateEnemies(deltaTime) {
    ...
    // Find which enemy is being aimed at, if any
    let aimedAtEnemy = null;
    const aimRaycaster = new THREE.Raycaster(playerPosition, playerDirection);
    const enemyMeshes = enemies.map(e => e.getObject());
    const intersectedEnemies = aimRaycaster.intersectObjects(enemyMeshes, true);

    if (intersectedEnemies.length > 0) {
        const closestIntersected = intersectedEnemies[0];
        // Traverse up to find the main enemy group
        let parent = closestIntersected.object.parent;
        while (parent && !parent.userData.isEnemy) {
            parent = parent.parent;
        }
        if (parent) {
            aimedAtEnemy = enemies.find(e => e.getObject().uuid === parent.uuid);
        }
    }
    ...
    enemies.forEach(enemy => {
        ...
        if (distanceToPlayer < ENEMY_SETTINGS.activationRadius) {
            ...
            const losRaycaster = new THREE.Raycaster(playerPosition, directionToEnemy, 0, distanceToPlayer);
            const intersects = losRaycaster.intersectObjects(worldObjects, true);

            let isVisible = true;
            if (intersects.length > 0) {
                // Check if the intersection is with the enemy itself or something in front
                const firstIntersectedObject = intersects[0].object;
                let intersectedEnemyObject = false;
                enemy.getObject().traverse(child => {
                    if (child === firstIntersectedObject) {
                        intersectedEnemyObject = true;
                    }
                });

                if (!intersectedEnemyObject && intersects[0].distance < distanceToPlayer - 0.5) {
                    isVisible = false;
                }
            }
        ...
        const isAimedAt = (aimedAtEnemy === enemy);
        enemy.update(deltaTime, playerPosition, isAimedAt, createProjectile);
    });

    if (closestVisibleEnemy && closestVisibleEnemy !== aimedAtEnemy) {
        const enemyObject = closestVisibleEnemy.getObject();
        const distanceToPlayer = playerPosition.distanceTo(enemyObject.position);

        if (distanceToPlayer > 5) { // Only move if not too close
            closestVisibleEnemy.walk();
            const moveDirection = new THREE.Vector3().subVectors(playerPosition, enemyObject.position).normalize();
            const speed = 1.5; // Adjust speed as needed
            enemyObject.position.addScaledVector(moveDirection, speed * deltaTime);
        } else {
            closestVisibleEnemy.stand();
        }
...
function updateElevators(deltaTime) {
    elevators.forEach(elev => {
        ...
        if (arrived) {
            ...
            elev.direction = 0; // Reset direction when arrived
        }

        // Update button glowing based on elevator direction
        const upButtonMaterial = elev.upButton.material;
        const downButtonMaterial = elev.downButton.material;
    ...
    for (let i = projectiles.length - 1; i >= 0; i--) {
        ...
        if (!projectile.userData.firedByPlayer) {
            ...
            if (playerBox.intersectsBox(projectileBox)) {
                ...
                break; // Player hit, no need to check other projectiles this frame
            }
        }
    }
