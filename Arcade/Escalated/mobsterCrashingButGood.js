...
export class Mobster {
    ...
    update(deltaTime, playerPosition, isPlayerVisible, isAimedAtByPlayer, createProjectileFunc, isActive = true) {
        ...
        if (!isActive) {
            ...
            if (this.isTurning) {
            ...
            } else if (isAimedAtByPlayer && (isPlayerVisible || isPlayerInFront)) {
                this.aimAt(playerPosition);
                this.fireProjectile(playerPosition, createProjectileFunc); // Fire when aiming
            } else {
                //this.fireProjectile(playerPosition, createProjectileFunc); // Fire when aiming
                ...
                if (this.patrolMinZ !== undefined && this.patrolMaxZ !== undefined) {
                    ...
                    if (wasAiming) {
                        // Adjust patrol direction based on player position
                        this.patrolDirection = this.patrolDirection * (isPlayerInFront ? 1 : -1); 
    ...
    fireProjectile(targetPosition, createProjectileFunc) {
        if (this.characterState !== 'aiming') return; // Change to only fire AFTER aiming

        const now = this.clock.getElapsedTime();
        if (now - this.lastShotTime < 2.0 || now - this.aimStartTime < this.aimTransitionDuration) { // Fire rate of 2 seconds // this.aimStartTime
            return;
        }
